...
def worker_task(args):
    ...
    while paths_generated < sets_per_map:
        ...
        if path:
            ...
            for y in range(height):
                for x in range(width):
                    # print(f"{(y*height+width)/(height*width)*100}")
...
def generate_and_save_maps_parallel(num_maps, width, height, density, directory="maps_test", sets_per_map=2):
    if not os.path.exists(directory):
        os.makedirs(directory)
    if not os.path.exists("paths_test"):
        os.makedirs("paths_test")
...
# # Example usage
num_maps = 1
width, height, density = 1000, 1000, 0.3
